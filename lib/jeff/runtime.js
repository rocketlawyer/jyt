...
// BEGIN(BROWSER)
    ...
    /*
    Jeff.getTemplateString = function(nameOrPath) {
        var t = Jeff.getTemplateStringByName(nameOrPath);
        if ( !t ) {
            t = Jeff.getTemplateStringByPath(nameOrPath);
        }
        return t;
    };
    */
    ...
    Jeff.VM = (function() {  // the main code that does the "compiling" and rendering
        ...
        var jtNameValue = function(name, val, parent) {
            ...
            if ( parent && (name === "attributes" || name === "attr" || name === "attrs" || name === "A") ) {
                ...
                // this template is meant to be rendered within another...
                // val is either the string/name/path of template to rendered inside of, and this is the first partial therein OR
                // val can be an object with { template: "/a/file/path.jeff", extensionPoint: "nameOfAPartial" }
                if ( Jeff.Utils.isObject(val) ) {
                    if ( val.template && val.template.length && val.template.length > 0 ) {
                        jtExtends = val.template;
                    }
                    if ( val.extensionPoint && val.extensionPoint.length && val.extensionPoint.length > 0 ) {
                        jtExtensionPoint = val.extensionPoint;
                    }
                } else if ( val && val.length && val.length > 0 ) {
                    jtExtends = val;
                }
        ...
        var includeMarkup = function(path, context) {
            ...
            if ( templateSpec ) {
                ...
                if ( !ret && Jeff.lastExtendedTemplate ) {
                    ret = Jeff.lastExtendedTemplate;
                    Jeff.lastExtendedTemplate = null;
                    console.log("took template from lastExtended: " + ret);
                }
            }
            return ret;
        };
